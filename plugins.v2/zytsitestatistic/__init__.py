...
class ZYTSiteStatistic(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "zytsitestatistic_"
    # 加载顺序
    plugin_order = 1
    # 可使用的用户级别
    auth_level = 2
    ...
    def __get_data(self) -> Tuple[str, List[SiteUserData], List[SiteUserData]]:
        ...
        empty_sites_name = []
        # 为空的 pop 出去
        for i, every_site_data in enumerate(yesterday_sites_data):
            if not every_site_data.upload:  # 上传量是 0
                removed = yesterday_sites_data.pop(i)
                empty_sites_name.append(removed.name)
        # 针对空数据的站点继续寻找近 3 天内有数的
        MAX_COUNT = 3
        ...
        while empty_sites_name and lookBackIndex <= MAX_COUNT:
            ...
            # 前 x 天的数据
            lookBackDay_sites_data = [data for data in data_list if data.updated_day == lookBackDay]
            for every_site_data in lookBackDay_sites_data:
                if every_site_data.name in empty_sites_name and every_site_data.upload > 0:
                    yesterday_sites_data.append(every_site_data)
                    empty_sites_name.remove(every_site_data.name)
