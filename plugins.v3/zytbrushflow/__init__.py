...
from app.schemas import TorrentInfo, MediaType, ServiceInfo
...
class BrushConfig:
    ...
    def __initialize_site_config(self):
        ...
        try:
            # site_config中去掉以//开始的行, bug 会把http:之后内容删除
            # site_config = re.sub(r'//.*?\n', '', self.site_config).strip()
            # 只匹配行首的注释
            site_config = re.sub(r'^\s*//.*?$', '', self.site_config, flags=re.MULTILINE).strip()
...
class ZYTBrushFlow(_PluginBase):
    ...
    def __get_downloading_uploading_count(self) -> int:
        ...
        try:
            ...
            if not downloader:
                return 0
            # torrents, error = downloader.get_torrents(tags=brush_config.brush_tag)
    ...
    def __get_subscribe_titles(self) -> Set[str]:
        """按 V3 媒体身份获取订阅标题，识别失败仍保留原名并允许下次重试。"""
        if not self.__get_brush_config().except_subscribe:
            return set()
        if self._subscribe_infos is None:
            self._subscribe_infos = {}
        subscribes = SubscribeOper().list() or []
        titles = set()
        current_keys = set()
        for subscribe in subscribes:
            name = (subscribe.name or "").strip()
            if name:
                titles.add(name)
            source, media_id = resolve_media_identity(media=subscribe)
            # 来源和 ID 变化时不能复用同名订阅的旧别名。
            key = (subscribe.id, name, source, media_id)
            current_keys.add(key)
            if key in self._subscribe_infos:
                titles.update(self._subscribe_infos[key])
                continue
            if not source:
                logger.warning("订阅媒体身份无效，保留原名用于排除")
                continue
            try:
                ...
                media = self.chain.recognize_media(
                    meta=meta, mtype=meta.type, media_source=source,
                    media_id=media_id, cache=True)
                if media:
                    aliases = {title.strip() for title in (media.names or [])
                               if title and title.strip()}
                    self._subscribe_infos[key] = aliases
                    titles.update(aliases)
                else:
                    logger.warning("订阅识别无结果，保留原名用于排除，下次重试")
            except Exception:
                logger.warning("订阅识别失败，保留原名用于排除，下次重试")
