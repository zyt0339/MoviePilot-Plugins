...
from app.schemas.types import EventType, MediaType, MediaImageType, MediaSource
from app.schemas.types import MessageType as NotificationType
from app.sdk.network import WebUtils


class ZYTMediaServerMsg(_PluginBase):
    """
    媒体服务器通知插件
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "zytmediaservermsg_"
    # 加载顺序
    plugin_order = 14
    # 可使用的用户级别
    auth_level = 1
    ...
    _ignore_usernames = None                   # 忽略的用户事件
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'type': 'warning',
                                            'variant': 'tonal',
                                            'text': '优先使用事件和媒体服务器提供的媒体身份；保留历史文件名识别标签。仅 TMDB 来源查询 TMDB 图片和详情，其他来源使用事件信息。'
                                        }
                                    }
                                ]
                            }
    ...
    def _handle_webhook_event(self, event: Event):
        ...
        try:
            ...
            if item_type != "MOV" and media_source in (None, MediaSource.TMDB):
            ...
            # 检查用户是否属于忽略的用户配置
            if self._ignore_usernames:
                if user_name in self._ignore_usernames.split(','):
                    logger.info(f"忽略的用户({user_name})事件,不发送消息:{message_title}")
                    return
            # 发送消息
    ...
    def _resolve_event_media_identity(
            self, event_info: Optional[WebhookEventInfo], lookup_item: bool = False
    ) -> Tuple[Optional[MediaSource], Optional[str]]:
        """按事件、路径、媒体服务器的顺序解析身份，不覆盖已有的非 TMDB 来源。"""
        if not event_info:
            return None, None
        source, media_id = resolve_media_identity(media=event_info)
        if source:
        ...
        if item_path:
            try:
                ...
                logger.debug("媒体路径身份解析失败，尝试媒体服务器")
        if lookup_item:
            try:
                service_info = self.service_info(name=getattr(event_info, "server_name", None))
                service = service_info.instance if service_info else None
                item_id = getattr(event_info, "item_id", None)
                if service and item_id:
                    return resolve_media_identity(media=service.get_iteminfo(item_id))
            except Exception:
                logger.debug("媒体服务器身份查询失败，使用事件原有信息发送通知")
        return None, None
    ...
    def _resolve_tv_tmdb_id_by_zyt(self, event_info: WebhookEventInfo,
        ...
        try:
            ...
            media_source, media_id = resolve_media_identity(media=series_info)
    ...
    def _send_aggregated_message_impl(self, series_id: str):
        ...
        try:
            ...
            media_source, media_id = self._resolve_event_media_identity(
                first_event, lookup_item=True)
            tmdb_id = media_id if media_source == MediaSource.TMDB else None
            if media_source == MediaSource.TMDB:
                tmdb_id = self._resolve_tv_tmdb_id_by_zyt(first_event, tmdb_id)
